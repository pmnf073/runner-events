              ...
              {editId === user.id && (
                <div style={{ background: V("--bg-input"), borderRadius: 8, padding: 16, marginTop: 12, display: "grid", gap: "8px" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                    ...
                    <div><label style={labelStyle}>Data de Nascimento</label><input type="text" value={(function(d){ if(!d) return ""; const datePart=(d||"").split("T")[0]; const [y,m,dd]=datePart.split("-"); return dd?`${dd.padStart(2,'0')}/${m.padStart(2,'0')}/${y}` : "" })(editForm.dob)} onChange={(e) => setEditForm({ ...editForm, dob: (function(v){ if(!v) return ""; if(v.includes('/')){ const p=v.split('/').map(x=>x.trim()); if(p.length!==3) return ""; return `${p[2]}-${p[1].padStart(2,'0')}-${p[0].padStart(2,'0')}` } return v })(e.target.value) })} placeholder="dd/mm/aaaa" style={inputStyle} /></div>
